...
#sys.path.append(r'C:\gdal232exe\bin\gdal\python\osgeo')
#from osgeo import gdal
sys.path.append(r'C:\gdal232exe\bin\gdal\python')
import gdal
...
def generate_resampled_anglebands(xml,resol):
    ...
    # determine a band reference for the output resolution
    if resol.endswith("10m"):
        #use band 4 as reference due to 10m spatial resolution
        imgref = [f for f in glob.glob(imgFolder + "**/*04.jp2", recursive=True)][0]
    elif resol.endswith("20m"):
        #use band 11 as reference due to 20m spatial resolution
        imgref = [f for f in glob.glob(imgFolder + "**/*11.jp2", recursive=True)][0]
    elif resol.endswith("60m"):
        #use band 1 as reference due to 60m spatial resolution
        imgref = [f for f in glob.glob(imgFolder + "**/*01.jp2", recursive=True)][0]
    ...
    sz_path = angFolder + scenename + '_solar_zenith_' + resol +'.tif'
    sa_path = angFolder + scenename + '_solar_azimuth_' + resol +'.tif'
    vz_path = angFolder + scenename + '_sensor_zenith_' + resol +'.tif'
    va_path = angFolder + scenename + '_sensor_azimuth_' + resol +'.tif'
...
def gen_s2_ang_from_SAFE(SAFEfile,resol):
    ...
    sz_path, sa_path, vz_path, va_path = generate_resampled_anglebands(xml,resol)
    return sz_path, sa_path, vz_path, va_path


def gen_s2_ang_from_xml(xml,resol):
...
def gen_s2_ang_from_zip(zipfile,resol):
...
def gen_s2_ang(path,resol):
    ...
    if path.find("_MSIL2A_") is not -1:
        ...
        sz_path, sa_path, vz_path, va_path = gen_s2_ang_from_SAFE(path,resol) #path to SAFE
    elif path.endswith('.xml'):
        sz_path, sa_path, vz_path, va_path = gen_s2_ang_from_xml(path,resol) #path MTD_TL.xml
    elif path.endswith('.zip'):
        sz_path, sa_path, vz_path, va_path = gen_s2_ang_from_zip(path,resol) #path to .zip
