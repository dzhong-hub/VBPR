...
%% ---------- Constants   
  ...
  %% --------- Defines menus for main window
    s2_process_menu = uimenu(main_window, ...
        ...
        'enable','on');
    s2_L2A_process_menu = uimenu(s2_process_menu, ...
        'label','Unzip S2 L1C data and Run L2A Process',...
        'separator','off', ...
        'enable','on', ...
        'callback','UnzipSen2CorS2L1C');
    s2_view_angles_menu = uimenu(s2_process_menu, ...
        'label','Interpolate Zenith and Azimuth Angles',...
        'separator','on', ...
        'enable','on', ...
        'callback','GenerateViewAngles');
    %% following code unifies a tile image, performance is very slow
    % s2_resample_menu = uimenu(s2_process_menu, ...
    %     'label','Unify S2 L2A Image Band Resolutions',...
    %     'separator','on', ...
    %     'enable','on', ...
    %     'callback','UnifyResolutionS2L2A');
    s2_gdal_subset_menu = uimenu(s2_process_menu, ...
        'label','Subset Tile Images by PCI Clip',...
        'separator','on', ...
        'enable','on', ...
        'callback','ClipSubAreaFromTileImageByPCIClip');
    s2_pci_clip_menu = uimenu(s2_process_menu, ...
        'label','Subset Tile Images by GDAL Translate',...
        'separator','on', ...
        'enable','on', ...
        'callback','ClipSubAreaFromTileImageByGDAL');
    s2_resample_subarea_menu = uimenu(s2_process_menu, ...
  ...
  vbpr_menu = uimenu(main_window, ...
    ...
    'enable','on');
    %% following cose estimates BIO variables for a tile image, don't work yet
    % sl2p_menu = uimenu(vbpr_menu, ...
    %     'label','Estimate By SL2P for a Tile',...
    %     'separator','off', ...
    %     'enable','on', ...
    %     'callback','VBPR_By_SL2P');    
    % alr_sl2p_menu = uimenu( vbpr_menu, ...
    %     'label','Estimate By ALR SL2P for a Tile',...
    %     'separator','off', ...
    %     'enable','on', ...
    %     'callback','VBPR_By_ALR_SL2P');
   sub_sl2p_menu = uimenu(vbpr_menu, ...
        'label','Estimate Bio Variables By SL2P for a Subset',...
        'separator','on', ...
        'enable','on', ...
        'callback','SUB_VBPR_By_SL2P');
   sub_alr_sl2p_menu = uimenu( vbpr_menu, ...
        'label','Estimate Bio Variables By ALR SL2P for a Subset',...
        'separator','on', ...
        'enable','on', ...
        'callback','SUB_VBPR_By_ALR_SL2P');        
    % sub_wv3_sl2p_menu = uimenu(vbpr_menu, ...
    %     'label','Estimate Bio Variables By SL2P with WV3 Images',...
    %     'separator','on', ...
    %     'enable','on', ...
    %     'callback','WV3_VBPR_By_SL2P');
    % sub_wv3_alr_sl2p_menu = uimenu( vbpr_menu, ...
    %     'label','Estimate Bio Variables By ALR SL2P with WV3 Images',...
    %     'separator','on', ...
    %     'enable','on', ...
    %     'callback','WV3_VBPR_By_ALR_SL2P');

  wv3_menu = uimenu(main_window, ...
    'label','WV2 Processing Tools', ...
    'enable','on');
%    sub_wv3_filter_menu = uimenu(wv3_menu, ...
%         'label','Blockmean WV3 Images to Selected Resolutions',...
%         'separator','on', ...
%         'enable','on', ...
%         'callback','SpatialFilterWV3Image');
   sub_wv3_match_menu = uimenu(wv3_menu, ...
        'label','Match WV3 Images to S2 Resolution (2m -> 20m)',...
        'separator','on', ...
        'enable','on', ...
        'callback','WV3_SampleMatchToS2Resolution');
   sub_wv3_calibrate_menu = uimenu(wv3_menu, ...
        'label','Calibration Regression (20m)',...
        'separator','on', ...
        'enable','on', ...
        'callback','WV3_Calibration_Regression');
   sub_wv3_apply_menu = uimenu(wv3_menu, ...
        'label','Application Regression (2m)',...
        'separator','on', ...
        'enable','on', ...
        'callback','WV3_Application_Regression');
   sub_wv3_biomatch_menu = uimenu(wv3_menu, ...
        'label','Resample WV3 BIO to S2 BIO Resolution (2m -> 10 / 20m)',...
        'separator','on', ...
        'enable','on', ...
        'callback','MatchWV3BioToS2BioResolution');
   sub_wv3_compare_bio_menu = uimenu(wv3_menu, ...
