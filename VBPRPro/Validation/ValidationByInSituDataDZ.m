function ValidationByInSituDataDZ(grnd_file, vbp_path, method, out_dir)
%
...
figure_0=figure('Color',[1 1 1],'position',[250,50,1400,900]); 
...
for ivar=["LAI","FCOVER","CWC"]
    ...
    fprintf(fid,'%s\n', [vbp_path '  ' num2str(STAT.RMSE,'%.2f') '  ' num2str(STAT.Bias,'%.2f') '  ' num2str(STAT.Slope,'%.2f') '  ' num2str(round(100*STAT.Acc), '%.d') '  ' num2str(STAT.N,'%.d')]);
