function ValidationByInSituData(grnd_file, vbp_path, method, out_dir)
%
...
%% open a file for statistic info comparisons
[path,fn,ext] = fileparts(grnd_file);
lai_file = [path '\CompLAI.txt'];
fcover_file = [path '\CompFCOVER.txt'];
cwc_file = [path '\CompCWC.txt'];
fid_lai = fopen(lai_file,'at');
fid_fcover = fopen(fcover_file,'at');
fid_cwc = fopen(cwc_file,'at');
...
datalist = dir([vbp_path,'*S2_MSI_BIO*.mat']);
s2=[];
...
for doy=GRND_doy'
    ...
    if abs(c)<2
       s2_1=importdata([vbp_path,'S2_MSI_BIO_SMAPVEX16_DOY_',num2str(s2_doy(idx)),'_T14UNA.mat']);
       s2_2=importdata([vbp_path,'S2_MSI_BIO_SMAPVEX16_DOY_',num2str(s2_doy(idx)),'_T14UNV.mat']);
...
%% remove the flags from the data
LAI=LAI(:,[1:7,9,11]);
...
%% average both values if they are valid in both images
LAI=[LAI(:,[1:6,9]),nanmean(LAI(:,7:8),2)];
FCOVER=[FCOVER(:,[1:6,9]),nanmean(FCOVER(:,7:8),2)];
CWC=[CWC(:,[1:6,9]),nanmean(CWC(:,7:8),2)];

%% distiguish the data into two groups LAI < 3 and LAI >= 3
...
figure_0=figure('Color',[1 1 1],'position',[50,0,1400,900]); 
...
for ivar=["LAI","FCOVER","CWC"]
    ...
    if ~isempty(data)
        ...
        %% append the statistics to comparison file
        if ivar=="LAI"
            fprintf(fid_lai,'%s\n', [vbp_path '  ' num2str(STAT.RMSE,'%12.2f') '  ' num2str(STAT.Bias,'%12.2f') '  ' num2str(STAT.Slope,'%12.2f') '  ' num2str(round(100*STAT.Acc), '%10.d') '  ' num2str(STAT.N,'%10.d')]);
        end
        if ivar=="FCOVER"
            fprintf(fid_fcover,'%s\n', [vbp_path '  ' num2str(STAT.RMSE,'%12.2f') '  ' num2str(STAT.Bias,'%12.2f') '  ' num2str(STAT.Slope,'%12.2f') '  ' num2str(round(100*STAT.Acc), '%10.d') '  ' num2str(STAT.N,'%10.d')]);
        end
        if ivar=="CWC"
            fprintf(fid_cwc,'%s\n', [vbp_path '  ' num2str(STAT.RMSE,'%12.2f') '  ' num2str(STAT.Bias,'%12.2f') '  ' num2str(STAT.Slope,'%12.2f') '  ' num2str(round(100*STAT.Acc), '%10.d') '  ' num2str(STAT.N,'%10.d')]);
        end
        ...
        text(0.05*def.lim_sup_data(3),0.8*def.lim_sup_data(3),[TXT_1,TXT_2],'FontName', 'Courier','FontSize',12,'FontWeight','normal','EdgeColor',[1 1 1],'BackgroundColor',[1 1 1],'Color',[0 0 0]);
        % -----------------------
        ax1=gca; 
...
savefig([out_dir, method, '_BIO_vs_SMAPVEX16.fig'])
saveas(figure_0,[out_dir, method, '_BIO_vs_SMAPVEX16.png'])
