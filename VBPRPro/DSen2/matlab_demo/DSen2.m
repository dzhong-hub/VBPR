...
x_tiles = ceil(im_size(1)/patchSize); %DZ:patchesAlongi 
y_tiles = ceil(im_size(2)/patchSize); %DZ:patchesAlongj

%% DZ: the following code cannot output correct or matched image size
% current_patch = 1;
% for y=1:y_tiles 
...
%% DZ: the following code is a fix to the code above
current_patch = 1;
for x=1:x_tiles  
    xpoint = (x - 1) * patchSize;        
    if xpoint > im_size(1) - patchSize
        x1 = xpoint+1; x2 = im_size(1);
        r1 = 1; r2 = (im_size(1)- xpoint);
    else
        x1 = xpoint+1; x2 = xpoint+patchSize;
        r1 = 1; r2 = patchSize;
    end   
    for y=1:y_tiles % DZ: fill the image along the colum direction
        ypoint = (y - 1) * patchSize;    
        if ypoint > im_size(2) - patchSize 
            y1 = ypoint+1; y2 = im_size(2);
            c1 = 1; c2 = (im_size(2)- ypoint);
        else
            y1 = ypoint+1; y2 = ypoint+patchSize;
            c1 = 1; c2 = patchSize;
        end    
        patch_block = squeeze(outtt(pad+1:sz(1)-pad, pad+1:sz(2)-pad, :, current_patch));
        q_image(x1:x2, y1:y2, :) = patch_block(r1:r2,c1:c2,:);
        current_patch = current_patch + 1;
    end
end
end
