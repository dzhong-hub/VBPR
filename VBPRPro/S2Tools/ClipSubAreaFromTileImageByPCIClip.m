function varargout = ClipSubAreaFromTileImageByPCIClip(varargin)
% CLIPSUBAREAFROMTILEIMAGEBYPCICLIP MATLAB code for ClipSubAreaFromTileImageByPCIClip.fig
%      CLIPSUBAREAFROMTILEIMAGEBYPCICLIP, by itself, creates a new CLIPSUBAREAFROMTILEIMAGEBYPCICLIP or raises the existing
%      singleton*.
%
%      H = CLIPSUBAREAFROMTILEIMAGEBYPCICLIP returns the handle to a new CLIPSUBAREAFROMTILEIMAGEBYPCICLIP or the handle to
%      the existing singleton*.
%
%      CLIPSUBAREAFROMTILEIMAGEBYPCICLIP('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CLIPSUBAREAFROMTILEIMAGEBYPCICLIP.M with the given input arguments.
%
%      CLIPSUBAREAFROMTILEIMAGEBYPCICLIP('Property','Value',...) creates a new CLIPSUBAREAFROMTILEIMAGEBYPCICLIP or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ClipSubAreaFromTileImageByPCIClip_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ClipSubAreaFromTileImageByPCIClip_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ClipSubAreaFromTileImageByPCIClip

% Last Modified by GUIDE v2.5 25-Apr-2021 18:11:45

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ClipSubAreaFromTileImageByPCIClip_OpeningFcn, ...
                   'gui_OutputFcn',  @ClipSubAreaFromTileImageByPCIClip_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ClipSubAreaFromTileImageByPCIClip is made visible.
function ClipSubAreaFromTileImageByPCIClip_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ClipSubAreaFromTileImageByPCIClip (see VARARGIN)

% Choose default command line output for ClipSubAreaFromTileImageByPCIClip
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes ClipSubAreaFromTileImageByPCIClip wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ClipSubAreaFromTileImageByPCIClip_OutputFcn(hObject, eventdata, handles) 
...
for n = 1:nf
    ...
    if exist(in_file, 'file')
        ...
        if (x1 >= xmax || x2 <= xmin || y1 >= ymax || y2 <= ymin)
            ...
            % Extract subarea image by PCI CLIP module
            % if ~empty(subarea_ref_file)
                 extractByPCIclip(in_file, subarea_ref_file, out_file);
            % else
...
%% close the log file
logmsg(flog,handles,'Croped subset image successfully!');
logmsg(flog,handles,['>>> Elapsed time: ', num2str(ttime),' seconds. <<<']);
fclose(flog);

disp('>>> Clipping subset is done. <<<');
...
% --- Executes on button press in pbSubset.
function pbSubset_Callback(hObject, eventdata, handles)
% hObject    handle to pbSubset (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
%% Clip all files by the PCI Clip Module in a LOOP
h = waitbar(0, 'Croping geotiff images for a subset...');
for n = 1:nf
    if ischar(file_list)
        in_file = file_list; 
    else
    ...
    [path, name, ext] = fileparts(in_file);
    out_file = [out_dir '\SUBSET_' name '.tif'];
