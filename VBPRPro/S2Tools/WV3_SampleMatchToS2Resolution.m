function varargout = WV3_SampleMatchToS2Resolution(varargin)
% WV3_SAMPLEMATCHTOS2RESOLUTION MATLAB code for WV3_SampleMatchToS2Resolution.fig
%      WV3_SAMPLEMATCHTOS2RESOLUTION, by itself, creates a new WV3_SAMPLEMATCHTOS2RESOLUTION or raises the existing
%      singleton*.
%
%      H = WV3_SAMPLEMATCHTOS2RESOLUTION returns the handle to a new WV3_SAMPLEMATCHTOS2RESOLUTION or the handle to
%      the existing singleton*.
%
%      WV3_SAMPLEMATCHTOS2RESOLUTION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in WV3_SAMPLEMATCHTOS2RESOLUTION.M with the given input arguments.
%
%      WV3_SAMPLEMATCHTOS2RESOLUTION('Property','Value',...) creates a new WV3_SAMPLEMATCHTOS2RESOLUTION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before WV3_SampleMatchToS2Resolution_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to WV3_SampleMatchToS2Resolution_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help WV3_SampleMatchToS2Resolution

% Last Modified by GUIDE v2.5 31-Mar-2021 16:26:56

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @WV3_SampleMatchToS2Resolution_OpeningFcn, ...
                   'gui_OutputFcn',  @WV3_SampleMatchToS2Resolution_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before WV3_SampleMatchToS2Resolution is made visible.
function WV3_SampleMatchToS2Resolution_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to WV3_SampleMatchToS2Resolution (see VARARGIN)

% Choose default command line output for WV3_SampleMatchToS2Resolution
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes WV3_SampleMatchToS2Resolution wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = WV3_SampleMatchToS2Resolution_OutputFcn(hObject, eventdata, handles) 
...
[filename, pathname] = uigetfile( ...
 ...
 if  filename > 0  
    ...
    if contains(s2_ref_file,'10m')
        out_file = strrep(filename, '.tif', '_10m.tif');
    elseif contains(s2_ref_file,'20m')
        out_file = strrep(filename, '.tif', '_20m.tif');
    else
...
%% get the option for the spatial smooth filter
opt_filter = get(handles.chkApplySmoothFilter, 'Value');
%% apply moving window spatial filter
if opt_filter == 1
    ...
    elseif out_res == 20
        ...
        msgbox('Incorrect resolution number in the reference file name!', 'error');
        return;
    end
    ...
    out_file = strrep(wv3_file,'.tif',['_' num2str(out_res) 'MPSF.tif']);
    wv3_bbox = geo.BoundingBox;
    geo.GTModelTypeGeoKey = 1;
    geotiffwrite(out_file, wv3_bbox, sf_img, 32, geo);
    
    % use the filtered image data for resampling 
    wv3_file = out_file;
    img = sf_img;
    clear sf_img;
end
...
if get(handles.popMatchMethod, 'Value') == 1
    ...
    out_file = strrep(wv3_file,'.tif',['_' num2str(out_res) 'MNNP.tif']);
    ...
    out_file = strrep(wv3_file,'.tif',['_' num2str(out_res) 'MAVG.tif']);
    % save the result with the reference geolocation info
    geotiffwrite(out_file, bbox, outimg, 32, option);
end
...
% --- Executes on button press in chkApplySmoothFilter.
function chkApplySmoothFilter_Callback(hObject, eventdata, handles)
% hObject    handle to chkApplySmoothFilter (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of chkApplySmoothFilter
