function varargout = WV3_Calibration_Regression(varargin)
% WV3_CALIBRATION_REGRESSION MATLAB code for WV3_Calibration_Regression.fig
%      WV3_CALIBRATION_REGRESSION, by itself, creates a new WV3_CALIBRATION_REGRESSION or raises the existing
%      singleton*.
%
%      H = WV3_CALIBRATION_REGRESSION returns the handle to a new WV3_CALIBRATION_REGRESSION or the handle to
%      the existing singleton*.
%
%      WV3_CALIBRATION_REGRESSION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in WV3_CALIBRATION_REGRESSION.M with the given input arguments.
%
%      WV3_CALIBRATION_REGRESSION('Property','Value',...) creates a new WV3_CALIBRATION_REGRESSION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before WV3_Calibration_Regression_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to WV3_Calibration_Regression_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help WV3_Calibration_Regression

% Last Modified by GUIDE v2.5 06-Feb-2021 22:37:46

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @WV3_Calibration_Regression_OpeningFcn, ...
                   'gui_OutputFcn',  @WV3_Calibration_Regression_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before WV3_Calibration_Regression is made visible.
function WV3_Calibration_Regression_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to WV3_Calibration_Regression (see VARARGIN)

% Choose default command line output for WV3_Calibration_Regression
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes WV3_Calibration_Regression wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = WV3_Calibration_Regression_OutputFcn(~, ~, handles) 
...
%% get wv3 input image files
wv3_calib_file = get(handles.txtWV3CalibrationImageFile,'String');

%% read in the sample data
img = geoimread(wv3_calib_file);
...
for bb={'LAI','FCOVER','FAPAR','LAI_Cab','LAI_Cw'}
    ...
    %% read in the flags and flg out the invalid pixels
    ...
    %% get calibration regression method
    switch get(handles.popRegressionMethod,'Value')
        case 1 % Linear Regression
            mdl.method = 'LinearR';        
            mdl.LinearR = fitlm(X,y,'RobustOpts','on');
            yhat = predict(mdl.LinearR, X);
            out_file = [out_dir '\' char(bb) '_LinearR.mat'];
            fig_file = [out_dir '\' char(bb) '_Calibration_LinearR.fig'];
            titletxt = 'Linear Regression';
        case 2 % Lasso Regression
            mdl.method = 'LassoR';
            % using standard Lasso regression
            % [mdl.LassoR, mdl.FitInfo] = lasso(X,y);
            [mdl.LassoR, mdl.FitInfo] = lasso(X,y,'Alpha',0.75,'CV',10);
            out_file = [out_dir '\' char(bb) '_LassoR.mat']; 
            fig_file = [out_dir '\' char(bb) '_Calibration_LassoR.fig'];
            idxLambda1SE = mdl.FitInfo.Index1SE;
            coef = mdl.LassoR(:,idxLambda1SE);
            coef0 =  mdl.FitInfo.Intercept(idxLambda1SE);
            yhat = X*coef + coef0;
            titletxt = 'Lasso Regression';
    ...
    if plot2comp > 0
        ...
        %fig_file = strrep(fig_file, '.tif', '_residue.fig');
        %png_file = strrep(fig_file, '.tif', '_residue.png');
        savefig(fig_file);
        saveas(gcf,png_file)
