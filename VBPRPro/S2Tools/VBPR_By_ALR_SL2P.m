function varargout = VBPR_By_ALR_SL2P(varargin)
% VBPR_BY_ALR_SL2P MATLAB code for VBPR_By_ALR_SL2P.fig
%      VBPR_BY_ALR_SL2P, by itself, creates a new VBPR_BY_ALR_SL2P or raises the existing
%      singleton*.
%
%      H = VBPR_BY_ALR_SL2P returns the handle to a new VBPR_BY_ALR_SL2P or the handle to
%      the existing singleton*.
%
%      VBPR_BY_ALR_SL2P('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VBPR_BY_ALR_SL2P.M with the given input arguments.
%
%      VBPR_BY_ALR_SL2P('Property','Value',...) creates a new VBPR_BY_ALR_SL2P or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before VBPR_By_ALR_SL2P_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to VBPR_By_ALR_SL2P_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help VBPR_By_ALR_SL2P

% Last Modified by GUIDE v2.5 24-Dec-2020 23:11:11

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @VBPR_By_ALR_SL2P_OpeningFcn, ...
                   'gui_OutputFcn',  @VBPR_By_ALR_SL2P_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before VBPR_By_ALR_SL2P is made visible.
function VBPR_By_ALR_SL2P_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to VBPR_By_ALR_SL2P (see VARARGIN)

% Choose default command line output for VBPR_By_ALR_SL2P
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes VBPR_By_ALR_SL2P wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = VBPR_By_ALR_SL2P_OutputFcn(~, ~, handles) 
...
[filename, pathname] = uigetfile( ...
 ...
 if  filename > 0  
    ...
    outpath = strrep(pathname, 'SR_IMG_DATA', 'VBPR_ALR');
...
logfile1 = [out_dir '\ALRSL2P_Process.log'];
...
%VBPR_ByALR(xmlfile, img_path, sl2p_path, vbp, out_dir); 
...
cur_dir = cd;
[filename, pathname] = uigetfile( ...
    {  '*.*','Specify file path to SL2P estimated variables (*.*)'; ...
    '*.*',  'All Files (*.*)'}, ...
    'Specify file path to SL2P estimated variables');
 if  filename > 0  
    set(handles.txtS2L2BDataInputPath, 'String', pathname);
    % set(handles.txtS2L2BDataInputPath, 'String', [pathname filename]);
 end 
 cd(cur_dir);
...
cur_dir = cd;
[filename, pathname] = uigetfile( ...
    {  '*.xml','Specify the S2L2A XML File (*.xml)'; ...
    '*.*',  'All Files (*.*)'}, ...
    'Specify the S2L2A XML File');
 if  filename > 0  
