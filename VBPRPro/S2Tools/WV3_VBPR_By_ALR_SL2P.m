function varargout = WV3_VBPR_By_ALR_SL2P(varargin)
% WV3_VBPR_BY_ALR_SL2P MATLAB code for WV3_VBPR_By_ALR_SL2P.fig
%      WV3_VBPR_BY_ALR_SL2P, by itself, creates a new WV3_VBPR_BY_ALR_SL2P or raises the existing
%      singleton*.
%
%      H = WV3_VBPR_BY_ALR_SL2P returns the handle to a new WV3_VBPR_BY_ALR_SL2P or the handle to
%      the existing singleton*.
%
%      WV3_VBPR_BY_ALR_SL2P('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in WV3_VBPR_BY_ALR_SL2P.M with the given input arguments.
%
%      WV3_VBPR_BY_ALR_SL2P('Property','Value',...) creates a new WV3_VBPR_BY_ALR_SL2P or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before WV3_VBPR_By_ALR_SL2P_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to WV3_VBPR_By_ALR_SL2P_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help WV3_VBPR_By_ALR_SL2P

% Last Modified by GUIDE v2.5 29-Jan-2021 15:00:32

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @WV3_VBPR_By_ALR_SL2P_OpeningFcn, ...
                   'gui_OutputFcn',  @WV3_VBPR_By_ALR_SL2P_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before WV3_VBPR_By_ALR_SL2P is made visible.
function WV3_VBPR_By_ALR_SL2P_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to WV3_VBPR_By_ALR_SL2P (see VARARGIN)

% Choose default command line output for WV3_VBPR_By_ALR_SL2P
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes WV3_VBPR_By_ALR_SL2P wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = WV3_VBPR_By_ALR_SL2P_OutputFcn(~, ~, handles) 
...
logfile1 = [out_dir '\WV3ALRSL2P_Process.log'];
...
logmsg(fid1,handles,['The input xml file: ' ref_file]);
...
VBPR_ByALR_WV3(ref_file, img_path, sl2p_path, vbp, out_dir); 
