function varargout = WV3_VBPR_By_Regression(varargin)
% WV3_VBPR_BY_REGRESSION MATLAB code for WV3_VBPR_By_Regression.fig
%      WV3_VBPR_BY_REGRESSION, by itself, creates a new WV3_VBPR_BY_REGRESSION or raises the existing
%      singleton*.
%
%      H = WV3_VBPR_BY_REGRESSION returns the handle to a new WV3_VBPR_BY_REGRESSION or the handle to
%      the existing singleton*.
%
%      WV3_VBPR_BY_REGRESSION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in WV3_VBPR_BY_REGRESSION.M with the given input arguments.
%
%      WV3_VBPR_BY_REGRESSION('Property','Value',...) creates a new WV3_VBPR_BY_REGRESSION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before WV3_VBPR_By_Regression_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to WV3_VBPR_By_Regression_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help WV3_VBPR_By_Regression

% Last Modified by GUIDE v2.5 03-Feb-2021 21:55:59

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @WV3_VBPR_By_Regression_OpeningFcn, ...
                   'gui_OutputFcn',  @WV3_VBPR_By_Regression_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before WV3_VBPR_By_Regression is made visible.
function WV3_VBPR_By_Regression_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to WV3_VBPR_By_Regression (see VARARGIN)

% Choose default command line output for WV3_VBPR_By_Regression
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes WV3_VBPR_By_Regression wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = WV3_VBPR_By_Regression_OutputFcn(~, ~, handles) 
...
logfile1 = [out_dir '\WV3RegressionProcess.log'];
if exist(logfile1,'file')
    delete logfile1;
end
fid1 = fopen(logfile1,'wt');
logmsg(fid1,handles,'Estimate Bio Variable from WV3 images by regression started.');

%% get the input and output folders
s2_sl2p_file = get(handles.txtS2SL2PBioVariableDataFile,'String');
logmsg(fid1,handles,['The input S2 SL2P Bio Variable image file: ' s2_sl2p_file]);
