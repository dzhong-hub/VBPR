function tiffile = SaveToGeotiffFile(img,out_fn,info,cropsubimg,overwrite,xlim,ylim)
...
%   cropsubimg - option 1 to crop a subarea from the tile image
%   overwrite - overwrite the tile image with the croped subarea image
%   xlim - UTM EAST coordinates range for the subarea
%   ylim - UTM NORTH coordinates range for the subarea
%
% Output:
...
%% clear data to free memories
clear img info;

%% crop image for defined subarea
if cropsubimg == 1
    tiffile = GetImageForDefinedArea(out_fn, xlim, ylim, overwrite);
else
    tiffile = out_fn;
end
