function VBPR_BySL2P_V1R(in_path, ang_path, reffile, out_path, vbp)
%
% VBPR by SL2P
%
% Input:
...
% reset the reference file for correct geolocation info if the output resolution is 20m
if strcmp(res, '20m')
    reffile = [in_path, '\', file_name];
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %% read in the view angle data
% file_name=dir([ang_path,'\*cosVZA_' res '.tif']);
% file_name=file_name(1).name;
% for bb={'cosVZA','cosSZA','cosRAA'}
%     file_name_band=strrep(file_name,'cosVZA',char(bb));
%     [band,xb,yb,Ib] = geoimread([ang_path,'\',file_name_band]);
%     [r,c]=size(band);
%     Input_NNT= [Input_NNT,double(reshape(band,r*c,1))]; 
% end
% Input_NNT(:,1:end)=Input_NNT(:,1:end)/10000;
% NNT_IN=[Input_NNT(:,end-2:end),Input_NNT(:,1:end-3)]';
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

waitbar(2/3)
%% 2.3 Organizing input data for NNET (NNET_IN)
% calculate relative azimuth angle = sensor_azimuth - solar_azimuth and
% delete the last colum 
Input_NNT(:,end-1)=abs(Input_NNT(:,end-1)-Input_NNT(:,end));Input_NNT(:,end)=[]; 
%Input_NNT(:,end-1)=Input_NNT(:,end-1)-Input_NNT(:,end);Input_NNT(:,end)=[];
% calculate cosVZA, cosSZA and cosRAA
Input_NNT(:,end-2:end)=cos(deg2rad(Input_NNT(:,end-2:end)));
% remove the scale factor for the reflectances
Input_NNT(:,1:end-3)=Input_NNT(:,1:end-3)/10000;
% map to the required input order: 
...
input_out_of_range=input_out_of_range_flag_function(Input_NNT(:,1:end-3),r,c);
...
for ivar=1:length(bio_vars)    
    ...
    %% initialize the output array by adding coordinates as the first two colums
    bio_sim=[reshape((1:r)'*ones(1,c),r*c,1),reshape(ones(1,r)'*(1:c),r*c,1), NaN+Input_NNT(:,1), NaN+Input_NNT(:,1)];

    %% simulate BIO variable
    eval(['NET_ivar= NET_estim.',bio,'.NET;']);    
    % bio_sim (:,3)= sim(NET_ivar, NNT_IN)';
    bio_sim (:,3)= NET_ivar(NNT_IN)';  
    
    %% simulate BIO variable uncertainties
    eval(['NET_unc= NET_uncer.',bio,'.NET;']);
    % bio_sim (:,3)= sim(NET_unc, NNT_IN)';
    ...
    bio_test = reshape(bio_sim(:,3),r,c);
    ...
    % [band,xb,yb,Ib] = geoimread(reffile);
    if license('test','MAP_Toolbox') == 1
        info = geotiffinfo(reffile); %% need mapping toolbox
        ...
        [img,x,y,info] = geoimread(reffile);
        Ib.BoundingBox=info.BoundingBox;
